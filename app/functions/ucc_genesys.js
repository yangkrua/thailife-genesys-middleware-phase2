...
const log = new xlog('./logs/ucc_genabandon', 'ucc_genabandon.log');
log.init();
...
let saveAbandonCallToSalesforce = async (dataAbandonList) => {
  log.info(`UCC dataAbandonList Result: ${JSON.stringify(dataAbandonList)}`);
  await salesForceService.callApiSaveAbandonCallSalesforce(dataAbandonList, environment);
  log.info("UCC saveAbandonCallToSalesforce Done");
};
