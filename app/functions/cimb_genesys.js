const config = require("../config/cimb_server.js");
...
const log = new xlog('./logs/cimb_genabandon', 'cimb_genabandon.log');
log.init();
...
let saveAbandonCallToSalesforce = async (dataAbandonList) => {
  log.info(`CIMB dataAbandonList Result: ${JSON.stringify(dataAbandonList)}`);
  await salesForceService.callApiSaveAbandonCallSalesforce(dataAbandonList, environment);
  log.info("CIMB saveAbandonCallToSalesforce Done");
};
