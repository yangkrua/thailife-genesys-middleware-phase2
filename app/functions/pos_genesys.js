const config = require("../config/pos_server.js");
...
let saveAbandonCallToSalesforce = async (dataAbandonList) => {
  log.info( `POS dataAbandonList Result: ${JSON.stringify(dataAbandonList)}` );
  await salesForceService.callApiSaveAbandonCallSalesforce(dataAbandonList,environment);
  log.info("POS saveAbandonCallToSalesforce Done");
};
...
let parserAbandonDetail = async (data, dataQueueIdObj) => {
  ...
  console.log( 'pos dataAbandonList Size :'+dataAbandonList.length );
  await log.info('pos dataAbandonList Size :'+dataAbandonList.length );
  await log.info(`pos ====== parserAbandonDetail->Done! =======`);
...
let parserPhoneNumber = async (mobile) => {
