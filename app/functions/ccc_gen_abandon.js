const config = require("../config/ccc_server.js");
...
const log = new xlog('./logs/ccc_genabandon', 'ccc_genabandon.log');
log.init();
...
let saveAbandonCallToSalesforce = async (dataAbandonList) => {
    log.info(`CCC dataAbandonList Result: ${JSON.stringify(dataAbandonList)}`);
    await salesForceService.callApiSaveAbandonCallSalesforce(dataAbandonList, environment);
    log.info("CCC saveAbandonCallToSalesforce Done");
};
                        ...
                        for (let index_p = 0; index_p < data.participants.length; index_p++) {
                            ...
                            if (
                                ...
                                if (dataAbandonList.length >= 100 && dataAbandonList.length % 100 == 0) {
                                    log.info("Wait for 20 seconds Rate limit exceeded the maximum api Genesys");
                                    await delay(30000);
                                }
