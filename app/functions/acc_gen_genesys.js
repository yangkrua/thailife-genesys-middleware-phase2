...
let ACC_VOICEMAIL = async(dataQueueIdObj) => {
      ...
      if (
        ...
        if (await lineCDR.length > 0) {
          ...
          // const localPath = await `${config.GENESES.data_process_inbox}/ACC_VOICEMAIL_${moment().format("YYYYMMDD_HHMMSS")}.txt`;
     ...
     // log.info( `API->postAnalyticsConversationsDetailsQuery(), PageNo=1, Result: ${JSON.stringify(dataResult)}` );
      ...
      if (
        ...
        if (await lineCDR.length > 0) {
          ...
          // const localPath = await `${config.GENESES.data_process_inbox}/ACC_CALLBACK_${moment().format("YYYYMMDD_HHMMSS")}.txt`;
          // await writeFile(localPath, lineCDR);
        }
      }
