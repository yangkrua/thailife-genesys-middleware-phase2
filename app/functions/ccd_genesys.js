const config = require("../config/ccd_server.js");
...
let saveAbandonCallToSalesforce = async (dataAbandonList) => {
  log.info( `CCD dataAbandonList Result: ${JSON.stringify(dataAbandonList)}` );
  await salesForceService.callApiSaveAbandonCallSalesforce(dataAbandonList,environment);
  log.info("CCD saveAbandonCallToSalesforce Done");
};
...
let parserAbandonDetail = async (data, dataQueueIdObj) => {
  ...
  console.log( 'ccd dataAbandonList Size :'+dataAbandonList.length );
  await log.info( 'ccd dataAbandonList Size :'+dataAbandonList.length );
  await log.info(`ccd ====== parserAbandonDetail->Done! =======`);
