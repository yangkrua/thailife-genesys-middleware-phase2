const config = require("../config/bay_server.js");
...
let saveAbandonCallToSalesforce = async (dataAbandonList) => {
  log.info( `BAY dataAbandonList Result: ${JSON.stringify(dataAbandonList)}` );
  await salesForceService.callApiSaveAbandonCallSalesforce(dataAbandonList,environment);
  log.info("BAY saveAbandonCallToSalesforce Done");
};
...
let parserAbandonDetail = async (data, dataQueueIdObj) => {
  ...
  console.log('bay dataAbandonList Size :'+dataAbandonList.length );
  await log.info('bay dataAbandonList Size :'+dataAbandonList.length );
  await log.info(`bay ====== parserAbandonDetail->Done! =======`);
