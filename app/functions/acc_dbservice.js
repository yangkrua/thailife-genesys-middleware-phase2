      ...
      dataList.map(async (element) => {
        ...
        if (data[0] != '') {
          ...
          if (result.recordset.length === 0) {
            ...
            log.info(`Inserted new row into IVR_ACC_VOICEMAIL with UCID: ${data[0]} `);
            ...
            log.info(`Updated row in IVR_ACC_VOICEMAIL with UCID:  ${data[0]} `);
          }
        }
      })
    );
  } catch (error) {
    log.error(`insertIvrAccVoicemail(), error: ${error} `);
  } finally {
...
async function insertIvrAccCallback(dataList) {
  try {
      ...
      dataList.map(async (element) => {
        ...
        if (data[0] != '') {
          ...
          if (result.recordset.length === 0) {
            ...
            log.info(`Inserted new row into IVR_ACC_CALLBACK with UCID: ${data[0]} `);
          } else {
            await pool
              .request()
              .input("UCID", sql.VarChar, data[0])
            ...
            log.info('Updated row in IVR_ACC_CALLBACK with UCID:', data[0]);
    ...
    log.error(`DB->insertIvrAccCallback(), error: ${error.message}`);
  } finally {
    // Close the SQL Server connection
    await sql.close();
  }
